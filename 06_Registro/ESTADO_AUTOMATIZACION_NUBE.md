...
- La automatizacion local `Oloverso` del PC esta pausada. No debe producir imagenes ni editar el worktree local mientras el flujo cloud este activo.
- La produccion futura debe ejecutarse solo en Codex Cloud cuando el limite de uso permita iniciar tareas.
...
- El siguiente item remoto registrado antes de la migracion Drive-first era `IMG-00027`, Ley 0006, Principio 1. Algunos worktrees locales pueden tener avances pendientes no empujados; no deben usarse como fuente de verdad mientras Cloud no los integre de forma explicita.

## Decision recomendada

Usar Codex Cloud + GitHub + Google Drive como sistema principal 24/7:
...
4. El PC local queda fuera de produccion: solo consulta, preparacion y supervision manual.

No usar Termux/movil como servidor principal salvo emergencia. Termux depende de bateria, red movil, gestion agresiva de procesos de Android y credenciales locales; es mas fragil que una ejecucion cloud.
...
2. Codex debe ejecutar la produccion en Codex Cloud, no contra archivos que solo existan en el PC.
...
7. La cola solo puede marcar `Correcta` cuando la imagen final exista como archivo independiente en Drive y este registrada en el manifiesto.

## Trabajo horario esperado en Codex Cloud
...
5. Subir la imagen correcta a Google Drive en la carpeta `Oloverso` como archivo PNG/JPG/WebP independiente.
...
- `AGENTS.md`: guia principal para Codex en cloud.
...
- `06_Registro/CAPACIDAD_DRIVE_UPLOAD.md`: compuerta de validez para upload real de imagenes a Drive.
...
- `tools/oloverso_drive_workflow.py`: helper para planificar lotes y registrar uploads.
...
La captura de 2026-05-30/31 muestra limite de uso de Codex alcanzado. Mientras ese limite este activo, Cloud no iniciara nuevas tareas aunque el prompt sea correcto. Esperar al restablecimiento indicado por la interfaz o ampliar capacidad.
